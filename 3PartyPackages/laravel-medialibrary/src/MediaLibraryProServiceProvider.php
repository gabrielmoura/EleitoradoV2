    ...
    public function registerRouteMacros()
    {
        Route::macro('mediaLibrary', function (string $baseUrl = 'media-library-pro') {
            Route::prefix($baseUrl)->group(function () {
                config('media-library.enable_vapor_uploads')
                    ? Route::post('post-s3', '\\'.MediaLibraryPostS3Controller::class)->name('media-library-post-s3')
                    : Route::post('uploads', '\\'.MediaLibraryUploadController::class)->name('media-library-uploads');
            });
        });
    }
}
